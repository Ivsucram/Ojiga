...
#Insertion must be made using Time.at(xxx) where xxx is the amount of time in seconds. So xxx = 60*60*24 is one day
	...
	validate :rentUnitPrice_LessOrEqual_totalValue
...
#TODO In the future take out this custom valdiation and make the numericality standard validation work
	...
	def rentUnitPrice_LessOrEqual_totalValue
		if (!rentUnitPrice.nil? and !totalValue.nil?) then
			if (rentUnitPrice > totalValue) then
				errors.add(:rentUnitPrice, 'greater than total value')
			end
		end
	end
